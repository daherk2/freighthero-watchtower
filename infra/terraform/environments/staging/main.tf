# FreightHero Watchtower - Staging Environment
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "watchtower/staging/terraform.tfstate"
    region         = "us-east-1"
    encrypt        = true
...
module "alb" {
  ...
  domain_name    = "staging-watchtower.freighthero.com"
  common_tags = {
    Project     = "FreightHero-Watchtower"
    Environment = "staging"
    ManagedBy   = "terraform"
...
output "api_url" {
  value = "https://staging-watchtower.freighthero.com"
}